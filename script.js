...
function calculateExpense() {
  ...
  {
    const expenseValueString=item.children[0].children[1].children[0].innerHTML.replace(/,/g, "");
 
    sumExpense+=parseFloat(expenseValueString);
  }
  totalExpense.innerHTML=formatMoney(sumExpense);
  return Math.abs(sumExpense);
}
calculateExpense();
...
function calculateBudget() {
  ...
  const availableBudget=(incomeTotal-expenseTotal).toLocaleString(undefined, { minimumFractionDigits: 2,});
...
function deleteEntry() {
 const itemForDelete=event.target.closest("li");
